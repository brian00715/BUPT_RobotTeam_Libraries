...
void vesc_set_duty(uint8_t controller_id, float duty) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_DUTY << 8), &msg);
}
...
void vesc_set_current(uint8_t controller_id, float current) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT << 8), &msg);
}
...
void vesc_set_current_brake(uint8_t controller_id, float current) {
    CANMsg msg = {0};
	buffer_append_int32(msg.ui8, (int32_t)(current * 1000.0));
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE << 8), &msg);
}
...
void vesc_set_rpm(uint8_t controller_id, float rpm) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_RPM << 8), &msg);
}
...
void vesc_set_pos(uint8_t controller_id, float pos) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_POS << 8), &msg);
}
...
void vesc_set_current_rel(uint8_t controller_id, float current_rel) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_REL << 8), &msg);
}
...
void vesc_set_current_brake_rel(uint8_t controller_id, float current_rel) {
    CANMsg msg;
	buffer_append_float32(msg.ui8, current_rel, 1e5);
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE_REL << 8), &msg);
}
...
void vesc_set_handbrake(uint8_t controller_id, float current) {
	...
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE << 8), &msg);
}
...
void vesc_set_handbrake_rel(uint8_t controller_id, float current_rel) {
    CANMsg msg;
	buffer_append_float32(msg.ui8, current_rel, 1e5);
	CAN_SendExtMsg(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE_REL << 8), &msg);
}
