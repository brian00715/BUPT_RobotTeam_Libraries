...
void BaseChassis_Init()
{
    ...
    YawPID.int_duty = 5.0;
    YawPID.ctrl_max = 5.0;
    Chassis_SpeedCtrlPID.Kp = 0.5;
    Chassis_SpeedCtrlPID.Ki = 0.01;
    Chassis_SpeedCtrlPID.Kd = 0;
    Chassis_SpeedCtrlPID.ctrl_max = 5.0;
    Chassis_SpeedCtrlPID.int_max = 10.0;
    Chassis_OmegaCtrlPID.Kp = 0.5;
    Chassis_OmegaCtrlPID.Ki = 0.0;
    Chassis_OmegaCtrlPID.Kd = 0;
    Chassis_OmegaCtrlPID.ctrl_max = 5.0;
...
{
    BaseChassis.TrackStatus.path_index = -1;
...
/** 
  * @brief 更新底盘位姿状态
  * @note  此函数调用频率与里程计坐标发布频率挂钩。在cmd.c的usart_exc_DMA_vega()函数使用DMA进行传输
  */
...
void Chassis_GoToPoint(Point2D target)
{
    ...
    float distance = sqrtf(pow(BaseChassis.PostureStatus.x - target.x, 2) +
                           pow(BaseChassis.PostureStatus.y, 2));
    ...
    {
        arrive_flag = 1;
        uprintf("arrive:%f,%f,%f\r\n", BaseChassis.PostureStatus.x, BaseChassis.PostureStatus.y, BaseChassis.PostureStatus.yaw);
...
void Chassis_MotionCtrl(void)
{
    ...
    float speed_ctrl = PID_Release(&Chassis_SpeedCtrlPID, BaseChassis.target_speed, now_speed);
    // float omega_ctrl = PID_Release(&Chassis_OmegaCtrlPID, BaseChassis.target_omega, now_omega);
    float omega_ctrl = BaseChassis.target_omega;
    BaseChassis.fChassisMove(speed_ctrl, BaseChassis.target_dir, omega_ctrl);
}
...
void Chassis_TrackVector(vec now_speed_vec, vec target_speed_vec, vec now_pos2next_target, vec distance_vec, float target_yaw)
{
    ...
    else
    {
        ...
        // 对法向修正向量进行控制
        float corr_vec_ctrl_x = -PID_Release(&NormalCorrPID_x, 0, corr_vec.x); // 需要使用两个PID结构体，否则last变量会被重用
        float corr_vec_ctrl_y = -PID_Release(&NormalCorrPID_y, 0, corr_vec.y);
        ...
        float omega = PID_Release(&YawPID, target_yaw, BaseChassis.PostureStatus.yaw); // 暂时不加入角速度内环
...
int Chassis_TrackPath(const PlanPoint point_sets[], int point_num)
{
    ...
    if (BaseChassis.TrackStatus.finish_track) // 轨迹跑完了
    {
        BaseChassis.target_speed = 0;
        BaseChassis.target_omega = 0;
    ...
    {
        BaseChassis.TrackStatus.finish_track = 1;
        uprintf("END\r\n");
    }
...
void Chassis_ResetTrackStatus()
{
    ...
    BaseChassis.TrackStatus.finish_track = 0;
    BaseChassis.target_speed = 0;
    //速度方向不应更改
    BaseChassis.target_omega = 0;
}
